...
import {
  ...
  type GptResearchPredictionOutputV01,
} from "./gpt-research-prediction-v0.1";

export const DEFAULT_GPT_RESEARCH_MODEL_V01 =
  "gpt-5.6-luna" as const;

export type GptResearchWebSourceV01 = {
  url: string;
...
export type OpenAiResearchPredictionResultV01 = {
  provider:
    "openai";
...
function getModel():
  ...
  return (
    process.env
      .OPENAI_LLM_MODEL
      ?.trim() ||
    DEFAULT_GPT_RESEARCH_MODEL_V01
  );
}
...
export async function generateOpenAiResearchPredictionV01(
  input:
    GptResearchPredictionInputV01,
): Promise<
  OpenAiResearchPredictionResultV01
> {
  const client =
    new OpenAI({
      apiKey:
