...
} from "../src/lib/ai/deepseek-research-prediction-v0.2";

import {
  DEFAULT_DEEPSEEK_RESEARCH_MODEL_V02,
  generateDeepSeekResearchPredictionV02,
} from "../src/lib/ai/deepseek-research-prediction-v0.2";

const DEEPSEEK_RESEARCH_MODEL_VERSION_V02 =
  "dictaziq-deepseek-research-prediction-v0.2" as const;

const RUNNER_VERSION =
  "dictaziq-deepseek-runner-v0.2";
...
const MODEL_DESCRIPTION =
  "DictazIQ independent DeepSeek pre-match prediction engine using structured DictazIQ and API-Football evidence; native web search is unavailable and probabilities remain uncalibrated.";
...
function modelCodeSha256():
  ...
  return canonicalSha256({
    ...
    deepSeekAdapter:
      sourceFile(
        "src/lib/ai/deepseek-research-prediction-v0.2.ts",
      ),
  });
}
...
async function main() {
  ...
  console.log(
    "DictazIQ DeepSeek Research Predictions",
  );
  ...
  console.log(
    `Model identity: ${DEEPSEEK_RESEARCH_MODEL_VERSION_V02}`,
  );

  console.log(
    `Research contract: ${DEEPSEEK_RESEARCH_PREDICTION_VERSION_V02}`,
  );

  console.log(
    `DeepSeek model: ${configuration.model}`,
  );
  ...
  assert.equal(
    sportIds.size,
    1,
    "DeepSeek fixture set contains multiple sports.",
  );
  ...
  if (
    ...
    await sql`
      ...
      VALUES (
        ${sportId}::uuid,
        ${DEEPSEEK_RESEARCH_MODEL_VERSION_V02},
        ${MODEL_DESCRIPTION},
        ${modelSha},
        ${JSON.stringify(
          configuration,
  ...
  if (
    ...
    assert.equal(
      modelRows.length,
      1,
      "DeepSeek model registration failed.",
    );
  }
  ...
  if (
    ...
    assert.equal(
      ...
      sportId,
      "DeepSeek model belongs to wrong sport.",
    );
    ...
    assert.equal(
      ...
      modelSha,
      "DeepSeek source changed under an existing model version.",
    );
    ...
    assert.equal(
      ...
      "DeepSeek configuration changed under an existing model version.",
    );
  }
  ...
  for (
    ...
    console.log(
      "ROUTE: independent DeepSeek analysis.",
    );
     ...
     *
     * once a DeepSeek baseline exists for this
     * fixture/model, do not create a duplicate.
     * Later DeepSeek refreshes should use a dedicated
     * revision path rather than rewriting the baseline.
     */
    const existingRows =
      await sql`
        SELECT
    ...
    if (
      ...
      console.log(
        `DEEPSEEK BASELINE EXISTING: ${existingRows[0].id}`,
      );
  ...
  Awaited<
    ReturnType<
      typeof generateDeepSeekResearchPredictionV02
    >
  > |
  null =
  null;
...
for (
  ...
  try {
    research =
      await generateDeepSeekResearchPredictionV02({
        purpose:
          "fallback",
...
if (
  ...
  continue;
     ...
     *
     * A DeepSeek response completing at/after kickoff
    ...
    if (
      ...
      console.log(
        "DISCARD: DeepSeek analysis completed at or after kickoff.",
      );
    ...
    console.log("");
    console.log(
      `DEEPSEEK FORECAST: ${research.prediction.selection.toUpperCase()}`,
    );
    console.log(
    ...
    console.log(
  `Over 1.5: ${research.prediction.over15.selection.toUpperCase()} | confidence=${research.prediction.over15.confidence} | evidence=${research.prediction.over15.evidenceGrade}`,
);

console.log(
  `Over 2.5: ${research.prediction.over25.selection.toUpperCase()} | confidence=${research.prediction.over25.confidence} | evidence=${research.prediction.over25.evidenceGrade}`,
);

console.log(
  `BTTS: ${research.prediction.btts.selection.toUpperCase()} | confidence=${research.prediction.btts.confidence} | evidence=${research.prediction.btts.evidenceGrade}`,
);
    ...
    /*
     * Freeze the immutable prediction input cutoff
     * at the database time immediately after the
     * DeepSeek response completes.
     */
    const inputSnapshot = {
      route:
        "deepseek_research",
    ...
    const output = {
      ...
      over15:
  research.prediction.over15,

over25:
  research.prediction.over25,

btts:
  research.prediction.btts,

/*
 * Backward-compatible projections for existing
 * DictazIQ UI/scorecard consumers.
 */
goalsView:
  research.prediction.over25.selection === "yes"
    ? "over_2_5"
    : "under_2_5",

bttsView:
  research.prediction.btts.selection,
    ...
    console.log(
      `DeepSeek response: ${research.responseId}`,
    );
    ...
    assert.equal(
      modelRows.length,
      1,
      "DeepSeek model is not registered.",
    );

    /*
     * Recheck for a concurrently created DeepSeek
    ...
    if (
      ...
      console.log(
        `DEEPSEEK BASELINE ALREADY CREATED: ${concurrentCheck[0].id}`,
      );
    ...
    assert.equal(
      insertedRows.length,
      1,
      "DeepSeek baseline insertion failed.",
    );
    ...
    console.log(
      `DEEPSEEK BASELINE INSERTED: ${predictionId}`,
    );
    ...
    if (
      ...
      console.log(
        `DEEPSEEK BASELINE PUBLISHED: ${timestamp(
          publication[0].published_at,
          "Publication",
        ).toISOString()}`,
      );
  ...
  console.log(
    `DeepSeek candidates: ${candidates}`,
  );

  console.log(
    `DeepSeek analysed: ${analysed}`,
  );

  console.log(
    `DeepSeek baselines inserted: ${inserted}`,
  );

  console.log(
    `DeepSeek baselines published: ${published}`,
  );
...
main().catch(
  (
    ...
    console.error(
      error instanceof Error
        ? `DeepSeek prediction run failed: ${error.message}`
        : "DeepSeek prediction run failed.",
    );
