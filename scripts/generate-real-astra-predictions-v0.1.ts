...
import {
  DEFAULT_ASTRA_RESEARCH_MODEL_V01,
  generateOpenAiAstraResearchPredictionV01,
} from "../src/lib/ai/openai-astra-research-prediction-v0.1";

const ASTRA_RESEARCH_MODEL_VERSION_V01 =
  "dictaziq-astra-research-prediction-v0.1" as const;

const RUNNER_VERSION =
  "dictaziq-astra-shadow-runner-v0.1";
...
const MODEL_DESCRIPTION =
  "DictazIQ GPT-6 Astra premium-competition shadow prediction engine using persisted pre-match evidence plus OpenAI web research; shadow-only, non-authoritative and uncalibrated.";

const ASTRA_PREMIUM_COMPETITIONS =
  [
...
function normalizedCompetitionValue(
  value: string | null,
): string {
  return (
    value ??
...
function isAstraPremiumCompetition(
  competition: string,
  country: string | null,
): boolean {
  const name =
    normalizedCompetitionValue(
      competition,
    );
...
function publishRequested():
  ...
  if (
    ...
    throw new Error(
      "Astra v0.1 is shadow-only. --publish is not allowed.",
    );
  }
...
function modelCodeSha256():
  ...
  return canonicalSha256({
    ...
    astraAdapter:
      sourceFile(
        "src/lib/ai/openai-astra-research-prediction-v0.1.ts",
      ),
  });
}
...
async function main() {
  ...
  console.log(
    "DictazIQ Astra Research Predictions",
  );
  ...
  console.log(
    `Model identity: ${ASTRA_RESEARCH_MODEL_VERSION_V01}`,
  );

  console.log(
    `Research contract: ${GPT_RESEARCH_PREDICTION_VERSION_V02}`,
  );

  console.log(
    `Astra model: ${configuration.model}`,
  );
  ...
  assert.equal(
    sportIds.size,
    1,
    "Astra fixture set contains multiple sports.",
  );
  ...
  if (
    ...
    await sql`
      ...
      VALUES (
        ${sportId}::uuid,
        ${ASTRA_RESEARCH_MODEL_VERSION_V01},
        ${MODEL_DESCRIPTION},
        ${modelSha},
        ${JSON.stringify(
          configuration,
  ...
  if (
    ...
    assert.equal(
      modelRows.length,
      1,
      "Astra model registration failed.",
    );
  }
  ...
  if (
    ...
    assert.equal(
      ...
      sportId,
      "Astra model belongs to wrong sport.",
    );
    ...
    assert.equal(
      ...
      modelSha,
      "Astra source changed under an existing model version.",
    );
    ...
    assert.equal(
      ...
      "Astra configuration changed under an existing model version.",
    );
  }
  ...
  for (
    ...
    console.log(
      "ROUTE: independent Astra analysis.",
    );
     ...
     *
     * once a Astra baseline exists for this
     * fixture/model, do not create a duplicate.
     * Later Astra refreshes should use a dedicated
    ...
    if (
      ...
      console.log(
        `ASTRA BASELINE EXISTING: ${existingRows[0].id}`,
      );
     ...
     * Astra forecast generation must never
    ...
    console.log(
      "Astra structured analysis: START",
    );

    let research:
  Awaited<
    ReturnType<
      typeof generateOpenAiAstraResearchPredictionV01
    >
  > |
  null =
  null;
...
for (
  ...
  try {
    research =
      await generateOpenAiAstraResearchPredictionV01({
        purpose:
          "fallback",
    ...
    const malformedJson =
      message.includes(
        "Astra returned invalid JSON for the research prediction.",
      );
    ...
    console.log(
      `ASTRA JSON ERROR: attempt ${attempt}/${maxJsonAttempts}.`,
    );
    ...
    if (
      ...
      console.log(
        "RETRY: requesting a fresh Astra structured response.",
      );
    ...
    console.log(
      "SKIP: Astra returned malformed JSON after retry budget.",
    );
...
if (
  ...
  continue;
     ...
     *
     * A Astra response completing at/after kickoff
    ...
    if (
      ...
      console.log(
        "DISCARD: Astra analysis completed at or after kickoff.",
      );
    ...
    /*
     * Public Astra forecasts require at least
     * low confidence and evidence grade C.
     *
     * Weak outputs remain useful for internal
    ...
    console.log("");
    console.log(
      `ASTRA FORECAST: ${research.prediction.selection.toUpperCase()}`,
    );
    ...
    console.log(
      `Astra response: ${research.responseId}`,
    );
    ...
    assert.equal(
      modelRows.length,
      1,
      "Astra model is not registered.",
    );

    /*
     * Recheck for a concurrently created Astra
    ...
    if (
      ...
      console.log(
        `ASTRA BASELINE ALREADY CREATED: ${concurrentCheck[0].id}`,
      );
    ...
    assert.equal(
      insertedRows.length,
      1,
      "Astra baseline insertion failed.",
    );
    ...
    console.log(
      `ASTRA BASELINE INSERTED: ${predictionId}`,
    );
    ...
    if (
      ...
      console.log(
        `ASTRA BASELINE PUBLISHED: ${timestamp(
          publication[0].published_at,
          "Publication",
        ).toISOString()}`,
      );
  ...
  console.log(
    `Astra candidates: ${candidates}`,
  );

  console.log(
    `Astra analysed: ${analysed}`,
  );

  console.log(
    `Astra baselines inserted: ${inserted}`,
  );

  console.log(
    `Astra baselines published: ${published}`,
  );

  console.log(
    `Astra baselines existing: ${existing}`,
  );



  console.log(
    `Non-premium fixtures excluded: ${nonPremium}`,
  );
...
main().catch(
  (
    ...
    console.error(
      error instanceof Error
        ? `Astra prediction run failed: ${error.message}`
        : "Astra prediction run failed.",
    );
