...
function modelCodeSha256():
  ...
  return canonicalSha256({
    ...
    deepSeekAdapter:
      sourceFile(
        "src/lib/ai/deepseek-research-prediction-v0.1.ts",
      ),
  });
}
...
async function main() {
  ...
  console.log(
    `Model identity: ${DEEPSEEK_RESEARCH_MODEL_VERSION_V01}`,
  );
  ...
  if (
    ...
    await sql`
      ...
      VALUES (
        ${sportId}::uuid,
        ${DEEPSEEK_RESEARCH_MODEL_VERSION_V01},
        ${MODEL_DESCRIPTION},
        ${modelSha},
        ${JSON.stringify(
          configuration,
...
if (
  ...
  continue;
    ...
    /*
     * Public DeepSeek forecasts require at least
