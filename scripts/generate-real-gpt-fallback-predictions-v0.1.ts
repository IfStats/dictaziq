...
const RUNNER_VERSION =
  "dictaziq-gpt-fallback-runner-v0.1";
...
function modelCodeSha256():
  ...
  return canonicalSha256({
    contract:
      sourceFile(
        "src/lib/ai/gpt-research-prediction-v0.1.ts",
      ),

    openAiAdapter:
      sourceFile(
        "src/lib/ai/openai-research-prediction-v0.1.ts",
      ),
  });
}
...
async function main() {
  ...
  console.log(
    `Model contract: ${GPT_RESEARCH_PREDICTION_VERSION_V01}`,
  );
